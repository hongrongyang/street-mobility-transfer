            ...
            for i in range(0, len(gpickle_files) - window_size + 1):
                ...
                for gpickle_file in chunk_files:
                    ...
                    for _, attr in graph.nodes(data=True):
                        ...
                        normalized_lat = (attr.get("lat") - self.global_lat_min) / (
                            self.global_lat_max - self.global_lat_min)
                        normalized_lon = (attr.get("lon") - self.global_lon_min) / (
                            self.global_lon_max - self.global_lon_min)

                        num_people = attr.get("num_people")
                        ...
                        precipitation = min(attr.get("precipitation"), 50)
                        ...
                        wind_speed = attr.get("wind_speed")
...
def get_dataloader(gpickle_dir, batch_size=12, input_dim=7, window_sizes=[12], num_workers=1):
    global_lat_lon_bounds = (33.705217850000004, 34.32986051290148, -118.66754966750842, -118.155479)
