...
with open("./POI_data/mapping/poi_type_mapping_la_to_sf_plus_nyc.pkl", "rb") as f:
    poi_types = pickle.load(f)

class CombinedSlidingWindowIterableDataset(IterableDataset):
    def __init__(self, gpickle_dirs, input_dim=7, window_sizes=None, batch_size=1, global_lat_lon_bounds=None, stride=6):
        ...
        self.stride = stride  # New stride parameter

        self.feature_ranges = {
            'num_people': {'min': 0.0, 'max': 2116},
            'temperature': {'min': 10.972, 'max': 33.857},
            'precipitation': {'min': 0.000, 'max': 3.551},
            'wind_speed': {'min': 0.038, 'max': 8.135}
        }
    ...
    def __iter__(self):
        for interval_id, (gpickle_dir, window_size) in enumerate(zip(self.gpickle_dirs, self.window_sizes)):
            ...
            for i in range(0, len(gpickle_files) - window_size + 1, self.stride):  # Use stride
                ...
                for gpickle_file in chunk_files:
                    ...
                    for _, attr in graph.nodes(data=True):
                        poi_type_idx = poi_types.get(attr.get("poi_type"), 596)
                        ...
                        num_people = min(attr.get("num_people", 0), 2116)
                        ...
                        precipitation = min(attr.get("precipitation", 0), 50)  # Cap at 50 mm/h
                        log_precipitation = math.log1p(precipitation)
                    ...
                    edge_features = [[attr.get("population_flow")] for _, _, attr in graph.edges(data=True)]
...
def get_dataloader(gpickle_dir, batch_size=2, input_dim=7, window_sizes=[12], num_workers=1, stride=6):
    global_lat_lon_bounds = (40.476578, 40.917630, -74.258843, -73.700233) 
