...
class CombinedSlidingWindowIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        window_sizes=None,
        batch_size=1,
        global_lat_lon_bounds=None,
        stride=6,
        skip_empty_edge=True,
        ...
        self.stride = stride
        self.skip_empty_edge = skip_empty_edge
        self.empty_cache_every_batch = empty_cache_every_batch
    ...
    def __len__(self):
        ...
        for gpickle_dir, window_size in zip(self.gpickle_dirs, self.window_sizes):
            ...
            if len(gpickle_files) < window_size:
                continue
            # RL: stride-based counting
            total_windows += (len(gpickle_files) - window_size) // self.stride + 1
    ...
    def __iter__(self):
        for interval_id, (gpickle_dir, window_size) in enumerate(zip(self.gpickle_dirs, self.window_sizes)):
            ...
            # RL: stride step
            for i in range(0, len(gpickle_files) - window_size + 1, self.stride):
                ...
                for gpickle_file in chunk_files:
                    ...
                    for _, attr in graph.nodes(data=True):
                        ...
                        log_precipitation = math.log1p(precipitation) if precipitation >= 0 else 0.0
                ...
                window_edge_index = torch.cat(edge_index_window, dim=1)
                window_edge_attr = torch.cat(edge_attr_window, dim=0).squeeze(-1)

                if self.skip_empty_edge and window_edge_index.size(1) == 0:
                    cumulative_node_count = 0
                    continue

                batch_node_window.append(x_window)
                batch_edge_index_window.append(window_edge_index)
                batch_edge_attr_window.append(window_edge_attr)
                ...
                if len(batch_node_window) == self.batch_size:
                    ...
                    batch_ei = torch.cat(batch_edge_index_window, dim=1)
                    batch_ea = torch.cat(batch_edge_attr_window, dim=0)

                    if self.skip_empty_edge and batch_ei.size(1) == 0:
                        batch_node_window, batch_edge_index_window, batch_edge_attr_window = [], [], []
                        cumulative_node_count = 0
                        if self.empty_cache_every_batch:
                            torch.cuda.empty_cache()
                            gc.collect()
                        continue

                    yield batch_x, batch_ei, batch_ea
...
def get_dataloader(
    ...
    global_lat_lon_bounds = (36.67740214528253, 36.90482235413335,
                             -119.91876568732503, -119.65546786772028)
